...
const shipments={
  ...
  done:[{id:'DLV-1038',type:'delivered',label:'تم التوصيل',title:'عباس العقاد، مدينة نصر',meta:'اليوم، 12:42 م',action:'عرض التفاصيل'}]
};
...
function deliveries(){const list=shipments[state.deliveryTab];return `<section class="screen">${header('التوصيلات','فرع مدينة نصر',false,'bell')}${offline()}<div class="screen-scroll"><div class="stack-lg"><article class="card active-count"><span class="info-icon">${icon('package',18)}</span><div><strong>طلبان نشطان</strong><p>يمكنك استلام طلبات إضافية دون حد أقصى</p></div></article><div class="tabs"><button data-tab="pending" class="${state.deliveryTab==='pending'?'active':''}">بانتظار الرد (1)</button><button data-tab="active" class="${state.deliveryTab==='active'?'active':''}">نشطة (2)</button><button data-tab="done" class="${state.deliveryTab==='done'?'active':''}">مكتملة</button></div><div class="stack">${list.map(shipmentCard).join('')}</div></div></div>${nav('deliveries')}</section>`}
...
function actionSheet(type){
  if(type==='result') return `<div class="sheet-scrim" data-action="close-sheet"><section class="action-sheet" role="dialog" aria-modal="true" aria-labelledby="result-title" onclick="event.stopPropagation()"><div class="sheet-handle"></div><h2 id="result-title">نتيجة التوصيل</h2><p>اختر النتيجة النهائية للطلب #DLV-1044</p><button class="option-card ${state.selectedOutcome==='delivered'?'selected':''}" data-outcome="delivered"><span class="info-icon" style="color:var(--emerald-700);background:var(--emerald-50)">${icon('check')}</span><div><strong>تم التوصيل بنجاح</strong><div class="caption">إغلاق الطلب وإيقاف التتبع</div></div></button><button class="option-card ${state.selectedOutcome==='failed'?'selected':''}" data-outcome="failed"><span class="info-icon" style="color:var(--red-700);background:var(--red-50)">${icon('alert')}</span><div><strong>فشل التوصيل</strong><div class="caption">يجب تحديد السبب</div></div></button><button class="option-card ${state.selectedOutcome==='returned'?'selected':''}" data-outcome="returned"><span class="info-icon" style="color:var(--orange-700);background:var(--orange-50)">${icon('refresh')}</span><div><strong>تم إرجاع الطلب</strong></div></button><button class="btn primary full" data-action="confirm-result" ${state.selectedOutcome?'':'disabled'}>متابعة</button></section></div>`;
...
document.addEventListener('click',e=>{
  ...
  if(action==='open-order')go('order');
  if(action==='result'||action==='reject'||action==='branch'){state.sheet=action;render()}
  if(action==='close-sheet'){state.sheet=null;render()}
  if(action==='accept-assignment'){state.orderStage='accepted';toast('تم قبول الطلب');go('order')}
  if(action==='confirm-result'){state.sheet=null;if(state.selectedOutcome==='delivered'){toast('تم تسجيل التسليم بنجاح');state.selectedOutcome='';go('deliveries')}else go('failure')}
  ...
  if(action==='start-delivery'){state.orderStage='transit';toast('بدأت عملية التوصيل');render()}
  if(action==='confirm-arrival'){state.orderStage='arrived';toast('تم تسجيل وقت وموقع الوصول');go('order')}
  if(action==='submit-failure'){if(!state.selectedReason)return;toast(state.selectedOutcome==='returned'?'تم تسجيل إرجاع الطلب':'تم تسجيل فشل التوصيل');state.selectedOutcome='';state.selectedReason='';go('deliveries')}
