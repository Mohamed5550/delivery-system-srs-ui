...
- يُدخل الموظف عنوان التوصيل كنص، ولا يحدد نقطة جغرافية على الخريطة عند إنشاء الطلب أو إسناده
- لا يتضمن تطبيق السائق خريطة داخلية؛ يعرض عنوان التوصيل النصي فقط، بينما يظل تتبع موقع السائق متاحًا للمحول والمشرف
...
- التاريخ
- القيمة
- في تبويب الطلبات المكتملة، تتوفر فترات سريعة: اليوم، آخر 7 أيام، وآخر 30 يومًا، إضافة إلى فترة مخصصة بتاريخ بداية ونهاية

**معيار القبول:**
1. تعمل جميع معايير التصفية
2. يمكن دمج معايير التصفية
3. تظهر النتائج بشكل صحيح
4. يكون الإعداد الافتراضي لتبويب الطلبات المكتملة هو "اليوم"
5. تعرض الواجهة الفترة المطبقة بوضوح وتحافظ عليها أثناء تصفح تفاصيل طلب ثم العودة إلى القائمة
6. لا يقبل النظام فترة مخصصة يكون تاريخ بدايتها بعد تاريخ نهايتها أو يكون تاريخ نهايتها في المستقبل
...
- لا يوجد حد أقصى لعدد الطلبات النشطة المسندة إلى السائق
...
| الحالة | الوصف |
| :--- | :--- |
| **متاح للإسناد** | السائق نشط ويمكن إسناد طلبات جديدة إليه بغض النظر عن عدد طلباته النشطة |
| **غير متاح للإسناد** | السائق غير نشط أو موقوف، ولا يمكن إسناد طلبات جديدة إليه |
...
#### BR-007: تعدد الطلبات النشطة
يمكن للسائق قبول وتنفيذ أي عدد من الطلبات النشطة في الوقت نفسه، ولا يفرض النظام حدًا أقصى على عددها. يعرض النظام عدد الطلبات النشطة للمعلومية فقط، ولا يستخدمه لمنع الإسناد أو القبول.
...
- يعرض عنوان التوصيل النصي بوضوح، ولا يتضمن خريطة داخلية أو يتطلب من المنشأة تحديد موقع جغرافي للطلب
- تكون إجراءات الطلب الجاري واضحة ومتسلسلة: إجراء أساسي واحد فقط في كل مرحلة (`بدء التوصيل` ثم `تأكيد الوصول` ثم `تسجيل النتيجة`)، وتوضع الإجراءات الثانوية بعيدًا عن الإجراء الأساسي
...
| **الفشل الخارجي** | التعامل مع فشل خدمات التتبع والرسائل |
...
| **الاستخدام** | عرض موقع السائق ومساره للمحول والمشرف فقط؛ لا تُعرض خريطة داخل تطبيق السائق |
| **المصادقة** | API Key |
| **البيانات المرسلة** | خط العرض وخط الطول الخاصان بالسائق |
| **البيانات المستقبلة** | بيانات عرض الخريطة والمسار المسجل للمحول والمشرف |
...
- الخريطة والملاحة داخل تطبيق السائق
- إثبات التسليم بالصور أو التوقيع
- نظام محاسبي كامل
- إدارة المخزون
...
| :--- | :--- |
| هل يمكن للسائق تنفيذ أكثر من طلب في نفس الوقت؟ | نعم، دون حد أقصى لعدد الطلبات النشطة وفق BR-007 |
...
| هل يحتوي تطبيق السائق على خريطة؟ | لا؛ يعرض عنوان التوصيل النصي فقط، والخريطة مخصصة للمحول والمشرف |
| هل يلزم إثبات تسليم بصورة أو توقيع؟ | لا؛ تسجيل نتيجة التوصيل كافٍ في الإصدار الحالي |
